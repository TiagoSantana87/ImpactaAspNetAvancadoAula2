...
namespace Loja.Mvc.Hubs
{
    public class LeilaoHub : Hub
    {
        public async Task Participar(string nomeParticipante, string produtoId)
        {
            ...
            Clients.Group(produtoId).adicionarMensagem(nomeParticipante, 
                Context.ConnectionId, "Bom leilão a todos!");
        }
        ...
        public void EnviarLike(string nomeRemente, string connectionIdDestinatario)
        {
            Clients.Client(connectionIdDestinatario).receberLike(nomeRemente);
        }
    }
}